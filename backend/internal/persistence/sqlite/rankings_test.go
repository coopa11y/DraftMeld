...
func TestReplaceRankingsIsAtomicPerSource(t *testing.T) {
	...
	defer store.Close()
	source := ranking.SourceDefinition{ID: "test", Name: "Test"}
	now := time.Now().UTC()
	...
	if err = store.ReplaceRankings(context.Background(), source, []ranking.Record{{SourceID: "test", PlayerKey: "two", Name: "Two", Position: "WR", Team: "BBB", Rank: 1}}, "tomorrow", now); err != nil {
	...
	if len(records) != 1 || records[0].PlayerKey != "two" {
