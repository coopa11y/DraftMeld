...
export function IdentityReviewQueue({ busy, issues, onBusyChange, onReviewed, onError }: IdentityReviewQueueProps) {
  ...
  async function resolve(issue: IdentityIssue, resolution: "confirmed-separate" | "merged") {
    const canonical =
      resolution === "merged" ? (canonicalByIssue[issue.issueKey] ?? issue.candidates[0]?.playerKey ?? "") : "";
    onBusyChange(true);
    onError("");
    try {
    ...
    <Panel variant="ranking" aria-labelledby="identity-review-heading">
      <div className="section-heading">
        <div>
          ...
          <h2 id="identity-review-heading">Player identity review</h2>
          <p className="section-description">
            Keep distinct people separate, or merge aliases that refer to the same player. A merge combines their
            ranking and projection signals under the selected canonical identity.
          </p>
        </div>
