  ...
  return (
    <section className="ranking-panel" aria-labelledby="identity-review-heading">
      ...
      <ul className="identity-list">
        {issues.map((issue) => <li key={issue.issueKey} className={issue.resolution ? "reviewed" : ""}><div><strong>{issue.reason}</strong><p>{issue.candidates.map((candidate) => `${candidate.name} (${candidate.position}, ${candidate.team})`).join(" / ")}</p>{issue.resolution === "merged" ? <span>Merged as {issue.candidates.find((candidate) => candidate.playerKey === issue.canonicalPlayerKey)?.name ?? issue.canonicalPlayerKey}</span> : null}</div>{issue.resolution ? <span>{issue.resolution === "merged" ? "Merged" : "Kept separate"}</span> : <div className="identity-actions"><label>Canonical player<select value={canonicalByIssue[issue.issueKey] ?? issue.candidates[0]?.playerKey ?? ""} onChange={(event) => setCanonicalByIssue((current) => ({ ...current, [issue.issueKey]: event.target.value }))} disabled={busy}>{issue.candidates.map((candidate) => <option key={candidate.playerKey} value={candidate.playerKey}>{candidate.name}</option>)}</select></label><button className="secondary-button" type="button" disabled={busy} onClick={() => void resolve(issue, "merged")}>Merge aliases</button><button className="secondary-button" type="button" disabled={busy} onClick={() => void resolve(issue, "confirmed-separate")}>Keep separate</button></div>}</li>)}
      </ul>
    </section>
  );
}
