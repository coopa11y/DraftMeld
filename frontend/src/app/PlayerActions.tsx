...
export function PlayerActions({ player, busy, primary = false, auction = false, inflation = 1, minimumBid = 1, maximumBid = Number.MAX_SAFE_INTEGER, onAction }: PlayerActionsProps) {
  ...
  useEffect(() => setCost(suggestedCost), [suggestedCost]);
  return (
    <div className={`player-actions${auction ? " auction-actions" : ""}`}>
      {auction ? <label><span>Winning bid for {player.name}</span><span className="currency-input">$<input type="number" min={minimumBid} max={maximumBid} step="1" value={cost} onChange={(event) => setCost(Number(event.target.value))} disabled={busy} /></span></label> : null}
